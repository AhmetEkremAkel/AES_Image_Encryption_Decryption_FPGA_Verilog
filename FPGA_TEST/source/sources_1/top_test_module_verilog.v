...
module top_test_module_verilog #(
    ...
    output wire        tx,
    output reg        done,
    output reg        [7:0]led, //this led outputs only for debug purposes
    input  wire       [7:0]sw   //this switch inputs only for debug purposes
);
    ...
    top_encryption encrypt_inst (
        ...
        .data_in  (data_in),
        .key_in   (128'h0001_0203_0405_0607_0809_0A0B_0C0D_0E0F), //Constat key for encryption module
        .nonce    (128'h0000_0000_1111_1111_0000_0000_0000_0000), //Constat nonce for encryption module
    ...
    top_decryption decrypt_inst (
        ...
        .key_in   (128'h0001_0203_0405_0607_0809_0A0B_0C0D_0E0F), //Constat key for decryption module
        .nonce    (128'h0000_0000_1111_1111_0000_0000_0000_0000), //Constat nonce for encryption module
    ...
    always @(posedge clk) begin
        ...
        if (reset) begin
            ...
            case (state)
            // ---------------------------------------------------- IDLE + RX--------------
            //Bu statede img_mem e input datasını kayıt ediyorum
            S_IDLE: begin
                done <= 0;
            ...
            // ----------------------------------------------------- Encryption -------
            //Tx den aldıgım ve img_mem'e kayıt ettigim verileri burada encrption modulune atacagım. Outputları cipher_text e kayıt edecegim
            S_RX_DONE: begin
            ...
            // ----------------------------------------------------- TX ------- 
            //cipher_text uart ile bilgisayara gönderiliyor
            S_TX_START : begin
                cipher_text[3] = connection[127:0];
                uart_tx_start <= 0;
            ...
            // ----------------------------------------------------- Decryption -------
            //Bu statede cipher_text decryption modulune giriyor ve outputu plain_text e yazılıyor
            S_TX_DONE : begin
                
                if (done_decryption == 1 && index < BLOCK_NUMBER - 2 ) begin
                    plain_text[index] <= data_out_reg[127:0];
                    index = index + 1;
                    connection_2 <= cipher_text[index];
                end
                else if (done_decryption == 1 && index == BLOCK_NUMBER - 2) begin
                    plain_text[index] <= data_out_reg[127:0];
                    connection_2 <= cipher_text[3];
                    index =  0;
                    index_2 = 0;
                    state <= S_TX_LAST;
                    start_decryption <= 0;
            ...
            // ----------------------------------------------------- TX -------
            //plain_text uart ile bilgisayara gönderiliyor
            S_TX_LAST : begin
                plain_text[3] <= data_out_reg[127:0];
                uart_tx_start <= 0;
                if ((index_2 <= 15 && uart_tx_done) || initial_start == 1) begin
            ...
            //128 bitlik veri alındı - sifrelendi - şifrelenmiş veri pc ye gönderildi - çözüldü - çözülmüş veri pc ye gönderildi
